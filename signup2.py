...
import json
import logging #remove in production
...
class PostJSON(BlogHandler):
    def get(self, post_id):
        ...
        if not post:
            ...
            response["subject"]= post.subject
            self.response.headers['Content-Type'] = "application/json"
            self.write(json.dumps(response))
        '''
        self.response.headers['Content-Type'] = "application/json"
        self.write('{"subject": "'+ str(post.subject) + '", "content": "' + str(post.content)+ '"}')
...
class FrontJSON(BlogHandler):
    def get(self):
        ...
        if posts:
            for post in posts:
                dict2 = {}
                dict2["subject"] = post.subject
                logging.debug(str(post.subject))
                dict2["content"] = post.content
                lista.append(dict2)
            self.response.headers['Content-Type'] = "application/json"
